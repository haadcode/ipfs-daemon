// TODO: make these tests use aegir

'use strict'

const path = require('path')
const assert = require('assert')
const IpfsDaemon = require('../src/ipfs-browser-daemon')

const dataDirectory = '/tmp/ipfs-daemon'
const defaultIpfsDirectory = './ipfs'

const TIMEOUT = 10000
// window.LOG = 'DEBUG' // set log level
  ...
  describe('starts a daemon', () => {
    ...
    it('with custom options', (done) => {
      let opts = {
        ...
        LogDirectory: '/tmp',
        // SignalServer: '0.0.0.0:12345', // re-enable this check once the double-callbackerror has been found and fixed
        Addresses: {
      ...
      ipfs.on('ready', (res) => {
        ...
        ipfs.config.get('Addresses.Swarm', (err, res) => {
          // assert.equal(res[0].startsWith('/libp2p-webrtc-star/ip4/0.0.0.0/tcp/12345/ws/ipfs/'), true)
          // assert.equal(signalServerAddr, true)
    ...
    it('with existing IPFS data directory', (done) => {
      ...
      ipfs1.on('ready', (res) => {
        ...
        ipfs2.on('ready', () => {
          ...
          ipfs1.config.get('Addresses.Swarm', (err, res) => {
            console.log("---", res)
            ipfs2.stop()
            done()
          })
